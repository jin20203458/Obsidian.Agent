...
###  Rule 6: 숫자 순서 형식 (`1. `, `2. ` 등) 및 코드 펜스 들여쓰기로 인한 마크다운 리스트 충돌 예방
노드 텍스트가 숫자로 시작하고 마침표와 띄어쓰기가 이어지거나 (예: `1. 공간 스캔`), ` ```mermaid ` 코드 펜스 블록이 마크다운 리스트(`1. `, `* `) 하위에 4공백 이상 들여쓰기(Indent)되면, **Obsidian 파서 엔진**이 이를 리스트 노드로 오인하여 `Unsupported markdown: list` 오류를 출력합니다. (GitHub 웹에서는 정상이어도 Obsidian에서 깨집니다.)
* **해결책**:
  1. 노드 텍스트 내 숫자 순서는 마침표 대신 둥근 괄호(예: `(1) 공간 스캔`), 괄호(예: `1) 공간 스캔`), 또는 HTML 엔티티(`1&#46;`) 형식으로 작성합니다.
  2. ` ```mermaid ` 코드 펜스는 마크다운 리스트 안으로 들여쓰지 말고 **문서 최외각 루트 레벨(0공백 들여쓰기)**로 배치합니다.

---

###  Rule 7: GitHub 뷰포트 (780px) 가로 폭 잘림 방지 (Max-Width & Label Shortening)
GitHub 웹 마크다운 영역의 최대 너비는 **약 780px**로 제한됩니다. 다이어그램의 가로 배치(`flowchart LR`)나 긴 노드 텍스트, `subgraph` 나열 등으로 가로 픽셀 폭이 780px을 초과하면 오른쪽 텍스트가 싹둑 잘리는(Clipped) 현상이 발생합니다.
* **주요 원인 및 주의사항**:
  * GitHub는 보안 정책(`securityLevel: strict`)으로 인해 노드 텍스트 내 `<br/>` 줄바꿈 태그를 무시하고 1줄로 길게 펼칠 수 있습니다.
* **해결책**:
  1. 노드 텍스트와 화살표 라벨은 **핵심 키워드/클래스명(마지노선 ~18자 이내) 중심으로 짧게** 작성하고, 세부 상세 설명은 다이어그램 바로 아래 본문 텍스트에 위임합니다.
  2. 가로 요소가 너무 많은 다이어그램은 `flowchart TD` (세로 배치)를 우선적으로 사용합니다.
...
| **순서 번호 / 리스트 충돌** | `A[1. 공간 스캔]` | `A["(1) 공간 스캔"]` 또는 `A["1&#46; 공간 스캔"]` | Obsidian `Unsupported markdown: list` 오류 방지 |
| **가로 폭 초과 잘림** | `A["(1) C# 대뇌 서버 고수준 Job 스케줄링 엔진"]` | `A["(1) C# 대뇌: Job 스케줄링"]` | GitHub 780px 가로 폭 잘림 방지 (키워드 축약 & 본문 위임) |
