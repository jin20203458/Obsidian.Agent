...
| **✅ Clang-Tidy 정탐 (TP)** | **6종** | **54건** | 수동 검출 8종 중 6종(54건)을 도구가 정밀 자동 포착 |
| **❌ Clang-Tidy 미탐 (FN)** | **2종** | **3건** | 포인터 앨리어싱 및 초기화 함수 분석 한계로 미탐 (L298, L131, L290) |
| **⚠️ Clang-Tidy 오탐 (FP)** | **1종** | **1건** | `thread_create` 인자 도메인 오독 (규격 외 오탐) |

* **💡 수동 검출 대비 도구 규칙 정탐률 (Recall):** **75.0%** (8종 중 6종 검출 성공)
* **📉 실질 결함 개소 정탐율 (Warning Match):** **94.7%** (수동 위반 개소 57건 중 54건 도구 적발 성공)
...
* **대상 코드**: `threads/synch.c` [L147-L157](file:///C:/Users/user/Desktop/test_src/threads/synch.c#L147-L157) (`sema_test_helper` 함수)
  ...
  static void sema_test_helper (void *sema_) {
      ...
      int i;
      for (i = 0; i < 10; i++) {
          sema_down (&sema[0]); // ⚠️ NULL 검증 가드 없는 역참조 (1건)
          sema_up (&sema[1]);   // ⚠️ NULL 검증 가드 없는 역참조 (1건)
      }
  }
  ```
* **위반 사유**: 인자로 들어온 `void *sema_` 포인터에 대해 NULL 가드 검증을 생략하고 다이렉트로 역참조 연산을 수행했습니다.
...
* **대상 코드**: `threads/synch.c` [L290-L299](file:///C:/Users/user/Desktop/test_src/threads/synch.c#L290-L299) (`cond_wait` 함수)
  ...
  void cond_wait (struct condition *cond, struct lock *lock) {
      ...
      list_push_back (&cond->waiters, &waiter.elem); // ⚠️ 스택 변수 주소를 외부 리스트에 등록 (1건)
      ...
  }
  ```
...
* **대상 코드**: 
  * [L131](file:///C:/Users/user/Desktop/test_src/threads/synch.c#L131): `struct semaphore sema[2];` (배열 선언 - 1건)
  * [L290](file:///C:/Users/user/Desktop/test_src/threads/synch.c#L290): `struct semaphore_elem waiter;` (구조체 선언 - 1건)
...
### ② `path-sensitive-arqa.AlwaysConstantCondition` (상수 조건식 금지) - **16건** (수동 규칙 19번)
* **검출 대상**: L69 (`while (sema->value == 0)`), L92 (`if (sema->value > 0)`), L116 (`if (!list_empty)`), L319 등 경로 분석 포함 16건
...
### ④ `ast-function-call-argument-consistency` (암묵적 형변환 및 인수 정합성) - **8건** (수동 규칙 26번)
* **검출 대상**: L135 `sema_init(..., 0)`, L181 `sema_init(..., 1)` 및 printf 첫 인수 L134 등 8건
...
### ⑤ `ast-extern-function-declaration` (외부 함수 extern 누락) - **20건** (수동 규칙 7번)
* **도구 경고**: `warning: 외부 함수 'list_init'는 호출 이전 선언에서 'extern'을 명시해야 합니다. [ast-extern-function-declaration]`
* **정밀 분석**: 외부 라이브러리 심볼 호출 시 명시적인 `extern` 선언 누락 지점을 칼같이 적발해 냈습니다 (총 20건).

### ⑥ `ast-multi-statement-per-line` (한 줄에 하나의 명령문 작성) - **5건** (수동 규칙 8번)
* **도구 경고**: `warning: 한 줄에 여러 개의 전역 선언(변수/함수/타입 등)이 있습니다. ... [ast-multi-statement-per-line]`
* **정밀 분석**: 함수 반환 타입 선언과 함수명이 행을 분리하지 않은 가독성 표준 미준수 라인(L60, L108 등 5건)을 적발하였습니다.

---

## ❌ 4. Clang-Tidy 미탐(False Negative) 상세 분석 (총 2종 3건)
*수동 분석에선 규격 위반이었으나 도구가 인지하지 못한 항목에 대한 구체적 코드 분석 결과입니다.*

1. **[포인터 32] 지역 스택 주소의 외부 유출 미탐 (`cond_wait` L298 - 1건):**
...
2. **[C-5] 구조체/배열 선언 시 초기화 생략 미탐 (L131, L290 - 2건):**
