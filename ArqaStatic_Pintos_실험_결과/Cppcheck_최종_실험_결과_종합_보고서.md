...
| **✅ Cppcheck 정탐 (TP)** | **4종** | **30건** | 수동 검출 8종 중 4종(30건)을 도구가 정밀 자동 포착 |
| **❌ Cppcheck 미탐 (FN)** | **4종** | **15건** | extern 누락, NULL 가드, 스택 유출 등 핵심 결함 미탐 |
| **⚠️ Cppcheck 오탐 (FP)** | **1종** | **2건** | 리스트 팝 널 포인터 역참조 오판 (규격 외 오탐) |

* **💡 수동 검출 대비 도구 규칙 정탐률 (Recall):** **50.0%** (8종 중 4종 검출 성공)
* **📉 실질 결함 개소 정탐율 (Warning Match):** **66.7%** (수동 위반 개소 45건 중 30건 도구 적발 성공)
...
* **위반 사유**: 인자로 들어온 `void *sema_` 포인터에 대해 NULL 가드 검증을 생략하고 다이렉트로 역참조 연산(`&sema[0]`)을 수행했습니다.
...
## ❌ 4. Cppcheck 미탐(False Negative) 상세 분석 (총 4종 15건)
*수동 분석에선 규격 위반이었으나 Cppcheck가 인지하지 못하고 놓친 미탐 내역입니다.*

1. **[스타일 07] 외부 함수 extern 선언 누락 미탐 (17건):**
   * 외부 함수가 사전 `extern` 명시 없이 다이렉트 호출되는 모든 지점에 대해 Cppcheck는 감지하지 못하고 통과시켰습니다.
2. **[스타일 11] else 블록 누락 미탐 (3건):**
   * 방어적 프로그래밍을 위한 else 블록이 누락된 제어 구조를 단속하지 못했습니다.
3. **[포인터 35] NULL 가드 누락 미탐 (1건):**
   * `sema_test_helper`에서 `void *sema_` 인자에 대해 NULL 가드 검증 생략 지점을 정적으로 탐지하지 못했습니다.
4. **[포인터 32] 지역 스택 주소 유출 미탐 (1건):**
   * 지역 변수 `waiter` 구조체의 스택 생명주기 탈출 경로를 Cppcheck 데이터 플로우가 추적해 내는 데 완전히 실패했습니다.
5. **[C-5] 구조체/배열 선언 시 초기화 생략 미탐 (2건):**
