...
대규모 롤플레잉 서사를 진행할 때, 모든 대화를 프롬프트에 누적하면 AI 모델의 컨텍스트 윈도우를 초과하여 심각한 비용 폭발과 품질 저하(Lost in the middle)를 유발합니다. 이를 방지하기 위해 `MemoryManagerService`를 통한 3단계 압축 아키텍처를 설계했습니다.
