...
### 3. 인과 캐스케이드(Causal Cascade) 벤치마크 시 지수형 노드 폭발로 인한 시스템 OOM 셧다운
...
#### 원인 (Root Cause)
* 인과 캐스케이드 벤치마크 생성 시 \(\sum_{k=0}^{D} C^k\) 등비수열 공식에 따라 깊이(D)=10, 자식 분기 수(C)=5일 때 총 노드 수가 **12,207,031개**로 지수 폭발함.
...
#### 해결책 (Resolution)
* 벤치마크 모드 및 실무 환경에서 인과 전파 깊이를 최대 3~5 사이의 안전 범위(노드 수 150~400개 규모)로 제한하여 RAM 폭사를 예방함.
* `05_csharp_ai_profiling.md` 실행 런북에 깊이 6 이상의 매개변수 금지 경고 문구를 안내함.
* (중장기 개선) 인과 전파 시 최대 탐색 뎁스를 강제 클램핑하는 상한 가드를 `BeliefEngine` 전파 모듈 단에 추가할 예정.
