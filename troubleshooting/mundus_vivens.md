...
## 2026-07-07: [Resolved] 생체 욕구 처리 중 무제한 가구 탐색 거리로 인한 영구 유턴 루프
...
## 2026-07-07: [Resolved] 허기/피로 욕구 비동기 엇박자로 인한 출발 즉시 유턴 (Journey Prep Desync)
...
## 2026-07-07: [Resolved] C# AI 대뇌와 C++ 물리 서버 간 자연어 계획(Intent) 키워드 불일치
...
## 2026-07-10: [Resolved] 도주 목적지 좌표의 맵 경계 이탈 및 (0, 0) 수렴 길찾기 에러
...
## 2026-07-10: [Resolved] 전투 중인 NPC의 일상 대화 및 고차원 스케줄 전환 인터럽트 결함
...
## 2026-07-12: [Resolved] 사망 개체에 대한 대화 트리거로 인한 20Hz 사망 처리 루프 스팸
...
## 2026-07-12: [Resolved] 2D Bake Map 음수 좌표계 누락으로 인한 Wilderness 오판정
...
* 음수 좌표 영역은 격자 인덱스 외부에 걸쳐 Bake가 누락되었으며, 실시간 위치 갱신 조회 시에도 강제로 `0`으로 클램핑되어 구역명이 무조건 `"Wilderness"`로 어긋나 출력됨.

### 3. 해결책 (Resolution)
* 2D 격자 맵의 양수 좌표계 제약에 부합하도록 `world_config.json`의 좌표를 안전한 양수 공간으로 보정함:
  * **늑대 서식지 (Wolf Nest)**: `(250.0, 0.0, -250.0)` ➔ **`(250.0, 0.0, 10.0)`**
  * **고블린 초소 (Goblin Outpost)**: `(-250.0, 0.0, 250.0)` ➔ **`(10.0, 0.0, 250.0)`**

---

## 2026-07-12: [Resolved] 맨바닥 생체 욕구 해결(노숙 식사/취침) 시 20Hz 로그 스팸 루프
...
### 3. 해결책 (Resolution)
* `ActionInteractFurniture::Tick`의 최초 진입 조건문에 바닥 식사/취침 행동 중인지를 가려내는 필터식을 보완하여, 이미 노숙 수면/식사 모션을 유지 중인 캐릭터는 점유 시도 단계를 스킵해 중복 로그 유발 없이 1회만 기록되도록 교정함.
  ```cpp
...
### 2. 원인 (Root Cause)
* Tracy 헤더를 `#ifdef TRACY_ENABLE` 가드 없이 무조건적으로 `#include <tracy/Tracy.hpp>`하고 있었음. `ENABLE_PROFILING=OFF` 시 CMake의 `target_include_directories`에 Tracy 경로가 추가되지 않으므로 컴파일러가 헤더를 찾지 못함.
...
### 2. 원인 (Root Cause)
* Tracy Profiler는 `TRACY_ENABLE` 매크로가 정의되면 전역 정적 객체 `tracy::Profiler`를 자동 생성함. 이 생성자는 `main()` 진입 이전의 **정적 초기화(Static Initialization) 단계**에서 실행되며 내부적으로 TCP 8086 포트 바인딩 및 소켓 인프라를 즉시 가동함.
* `TRACY_ON_DEMAND` 매크로가 정의되지 않은 경우(vcpkg 기본값), GUI 연결 여부와 무관하게 무조건 전체 소켓 인프라를 초기화함. 이 과정에서 초기화가 실패하면(소켓 권한, 방화벽 차단 등) Tracy 내부에서 `exit(1)`이 호출되어 콘솔 출력 0바이트 상태로 프로세스가 종료됨.

### 3. 해결책 (Resolution)
* `CMakeLists.txt`에서 `target_compile_definitions`를 통해 컴파일 타임 매크로 `TRACY_ON_DEMAND`를 빌드 타겟에 직접 주입하여, 프로파일러 GUI가 실제로 접속하기 전까지 소켓 인프라 초기화를 지연시킴.
* 검증: `ENABLE_PROFILING=ON` + `TRACY_ON_DEMAND` 빌드 후 정상 실행 및 Tracy GUI(`localhost:8086`) 정상 연동 확인.
...
* Windows 방화벽 및 로컬 보안 정책 제한으로 인해, 서버 기동 시 Tracy가 시도하는 UDP 브로드캐스트 패킷 바인딩 작업이 실패하여 프로세스가 강제 차단됨.
...
### 3. 해결책 (Resolution)
1. **의존성 소스 코드 프로젝트 격리 내재화**: vcpkg 사전 빌드 라이브러리 링크를 우회하고 매크로를 안전하게 컴파일러에 제어하기 위해, Tracy 클라이언트 구현 소스 전체를 프로젝트 내부 `thirdparty/tracy` 폴더 하위로 완벽하게 복사 및 격리 임포트함.
2. **타겟 레벨 매크로 및 include 격리**: CMakeLists.txt에서 `TracyClient.cpp`를 타겟 소스 파일로 지정하고, `target_compile_definitions`를 통해 `TRACY_ENABLE`, `TRACY_ON_DEMAND`, `TRACY_NO_BROADCAST` 매크로를 격리 주입하여 UDP 연산을 원천 차단하고 순수 TCP 포트(8086)만 오픈하도록 통제함.
3. **검증**: `netstat -ano` 확인 시 `TCP 0.0.0.0:8086` LISTENING 상태 정상 확인 및 `tracy-profiler.exe` 프레임 타임라인 수집 검증 완료.

---

## 2026-07-15: [Resolved] DB 강제 리셋 시 cold_archive 컬렉션 Drop 누락으로 인한 더미 데이터 간섭
...
* 이 더미 기억(`SubjectId = "target_npc"`)을 에바가 대화 시작 시 리콜하여 대뇌 프롬프트에 실으려 하다가, `target_npc`에 해당하는 실제 에이전트 인스턴스가 없어 Null Key 에러를 유발함.
...
## 2026-07-15: [Resolved] 비지성체(야수/몬스터)에 대한 대화 트리거 필터 누락 결함
...
* 그러나 C++ 대화 판정 엔진인 `SystemSocial.cpp`에서 대화 참여 주도자를 수집할 때와 주변 타겟 이웃을 스캔하여 매칭할 때 `SentienceComp` 검사가 빠져 있어 늑대가 정상 대화 상대로 매칭됨.
...
* 더불어 `childId` 포맷이 `$"belief_level_{currentDepth}_child_{i}_{parentId}"`와 같이 부모 ID 문자열을 재귀 중첩하는 방식으로 설계되어 있어 트리가 깊어질수록 문자열 바이트 크기가 거대해짐.
* 수백 바이트짜리 ID 키를 포함한 1,220만 개의 `Belief` 객체가 C# 인메모리에 일시에 동시 할당되면서 GC가 메모리를 해제하지 못하고 셧다운을 유발함.

### 3. 해결책 (Resolution)
1. **깊이 제한 가드 (Depth Clamping Guard = 5)**: `BeliefEngine.cs` 내부의 `PropagateCausalCascade`에서 전파 깊이가 5레벨을 초과하여 6레벨에 다다르면 더 이상 연산을 진행하지 않고 조기 종료시켜 지수 노드 폭발을 원천 차단함.
2. **순환 참조 방지 가드 (Circular Dependency Guard)**: 신념이 서로 꼬리를 물어 재귀 순환(예: A ➔ B ➔ A) 궤도에 오를 때, `HashSet`을 통해 방문 상태를 검증하고 즉각 탈출하도록 구현하여 StackOverflowException을 예방함.
3. **가이드 런북 명시**: 벤치마크 실행 가이드에 안전 범위(깊이 3~5)를 명시하여 시스템 과부하를 방지함.
...
### 3. 해결책 (Resolution)
* C++ 게임 서버 빌드 시 CMakeLists.txt에 Windows 멀티미디어 정밀 타이머 라이브러리인 `winmm`을 타겟으로 링크함.
* `main.cpp` 상단에 컴파일 충돌 방지를 위해 `#define WIN32_LEAN_AND_MEAN` 처리와 함께 `<Windows.h>` 및 `<timeapi.h>`를 조건부 include 함.
...
### 2. 원인 (Root Cause)
* MemoryBox 용량 상한선(40개) 초과 시 오래된 기억이 도태(Evict)되어 LiteDB의 `cold_archive` 컬렉션에 보관됨. 이 과정에서 `OnBeliefEvicted` 핸들러가 메인 스레드상에서 디스크 파일(`GameData.db`)에 동기식(Synchronous)으로 쓰기를 수행함.
* 또한 `MemoryBox._lock`을 잡은 상태에서 `PersistenceService._dbLock`까지 순차적으로 획득하여 이중 락 중첩(Double-Lock Nesting) 상태로 디스크 I/O를 대기하여 메인 루프를 블로킹함.

### 3. 해결책 (Resolution)
* `PersistenceService.cs`에 `System.Threading.Channels` 기반의 Async Write-Behind Queue(`Channel<ArchiveEntry>`, 크기 2048, `DropOldest` 바인딩)를 구축함.
* 동기식 직접 쓰기 대신 O(1) 시간 복잡도의 `EnqueueArchive` 메모리 큐 삽입으로 전환하여 메인 스레드의 대기 시간을 제거함.
* 백그라운드 워커 스레드(`RunArchiveWorkerAsync`)를 구동하여 큐의 내용을 LiteDB에 비동기 일괄 기록하도록 아키텍처를 분리함.
* `Dispose` 시 채널을 닫고 최대 10초간 대기하며 잔여 큐를 완전히 Flush 하도록 구현하여 Graceful Shutdown 정합성을 보장함.

---

## 2026-07-20: [Resolved] 감쇠(Decay) 삭제된 기억의 콜드 아카이브 누락 및 영구 유실 해결

### 1. 현상 (Symptom)
* 시간이 경과하여 기억 선명도(Salience)가 0.1 미만으로 감쇠되어 도태된 기억들이 콜드 아카이브 DB에 보관되지 않고 영구 삭제되는 데이터 유실 현상 발생.

### 2. 원인 (Root Cause)
* `BeliefEngine.cs`의 `DecayBeliefs()` 내에서 감쇠된 기억을 지울 때, `OnBeliefEvicted` 이벤트를 호출하지 않고 메모리 상의 `ConcurrentDictionary`에서 직접 `TryRemove` 처리하여 DB 이관 과정을 우회함.

### 3. 해결책 (Resolution)
* `BeliefEngine.cs`에서 `TryRemove` 성공 시 `agent.MemoryBox.OnBeliefEvicted?.Invoke(removed)` 훅을 호출하도록 수정함.
* 훅 호출 처리가 비동기 지연 쓰기 큐를 경유하므로 감쇠 감지 시에도 서버 프레임 지연 유발 없이 cold_archive 데이터베이스에 정합성 있게 보관됨.
