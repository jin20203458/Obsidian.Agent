...
* AI 지연이나 장애 발생 시 ntdll 로더 락 데드락을 방지하는 **30초 `SafetyWatchdog` 자가 회복 가드**를 연동합니다.
...
| **원자적 선제 동결 지연 (Suspend)** | 감지 후 **50μs 미만** (실측 **24~27μs**) | `ntdll!NtSuspendProcess` 동적 로딩 및 30초 세이프티 워치독 |
