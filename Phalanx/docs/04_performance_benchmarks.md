...
### E. 최신 세대 모델(`gemini-3.8-flash`) 10회 연속 실측 벤치마크 (Ground Truth)

동일한 시나리오 및 통제 조건 하에서 타깃 모델을 **`gemini-3.8-flash`**로 승격하여 3대 아키텍처 각 10회(총 30회 세션) 연속 벤치마크를 재실측하였습니다.

| 평가 메트릭 (10회 실측) | 방식 1 (Current JSON Mode) | 방식 2 (responseSchema) | 방식 3 (Native Function Calling / MCP) |
| :--- | :---: | :---: | :---: |
| **파싱 및 실행 성공률** | **7 / 10 (70.0%)** | 6 / 10 (60.0%) | **0 / 10 (0.0% 머신 판정 불가)** |
| **평균 총 소요시간 (Mean)** | **`9,960.8 ms` (~9.9초, 1위)** | `10,351.6 ms` | **`11,024.2 ms` (~11.0초, 최하위)** |
...
| **필수 네트워크 왕복 횟수** | **단 1 회 (One-Shot 완결)** | **단 1 회** | **최소 2 회 (Multi-Turn 필수)** |
| **도구 선택 정확도 (DecodePayload)** | **70.0%** | 60.0% | 60.0% |
| **필수 인자(`encodedCommand`) 전달율** | **70.0%** | 60.0% | 60.0% |
| **사고 과정(`Thought`) 평균 글자 수** | **평균 202 자 (풍부한 보안 소명)** | 128 자 | **0 자 (10회 전회 완전 누락)** |
| **보안 맥락 키워드 포함 비율** | **70.0%** | 60.0% | **0.0%** |
| **평균 토큰 소비량 (입력 / 출력)** | 447 / 378 토큰 | 605 / 276 토큰 | **345 / 109 토큰** |
| **최종 사살(`ACTION_KILL`) 판결율** | **60.0%** | 60.0% | **0.0% (줄글 자연어 응답)** |

#### [Gemini 3.8 Flash 실측 핵심 인사이트]
1. **지연시간 역전 현상 규명 (방식 1이 방식 3보다 빠름)**:
   - `gemini-3.8-flash` 환경에서는 **방식 1(평균 9.9초)**이 **방식 3(평균 11.0초)**보다 **1.06초 더 빠른 역전 현상**이 실측되었습니다.
   - 방식 3은 멀티턴 왕복(Turn 1 도구 호출 $\rightarrow$ 클라이언트 실행 $\rightarrow$ Turn 2 최종 응답) 과정에서 모델의 첫 토큰 생성 지연(TTFT)과 네트워크 RTT가 2회 누적되고, 후반부 타임아웃(15초 컷)이 발생하며 평균 11초로 가장 느린 아키텍처로 기록되었습니다.
2. **사고 과정(Thought) 거세 문제 재현**:
   - `gemini-3.8-flash`에서도 방식 3(Function Calling)은 10회 중 10회 모두 **사고 과정이 0자(완전 누락)**로 나타나 보안 소명 불능 문제가 지속되었습니다. 반면 방식 1은 평균 202자의 정확한 공격 맥락(winword 매크로, Base64 난독화, MITRE T1059 등)을 명시했습니다.
3. **머신 판정 집행률 0% 결함 유지**:
   - 방식 3의 Turn 2 응답은 여전히 서술형 자연어로 반환되어, C# EDR 커널 액추에이터가 즉시 사살할 수 있는 머신 코드(`ACTION_KILL`) 변환율이 **0%**에 그쳤습니다.

### F. 최종 아키텍처 채택 결론
Phalanx EDR의 AI 수사 파이프라인은 2.5 Flash 및 3.8 Flash 전 세대에 걸친 60여 회 누적 실측 데이터(Ground Truth)에 기반하여 **[방식 1: JSON Mode + LlmJsonParser + C# 런타임 언래핑 & 로컬 체이닝]**을 단일 표준 아키텍처로 최종 확정합니다.
